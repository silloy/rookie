package me.silloy.study.websocket.netty.yeauty.standard;

import me.silloy.study.websocket.netty.yeauty.annotation.ServerEndpoint;
import me.silloy.study.websocket.netty.yeauty.pojo.PojoEndpointServer;
...
public class ServerEndpointExporter extends ApplicationObjectSupport implements SmartInitializingSingleton, BeanFactoryAware {
    ...
    protected void registerEndpoints() {
        Set<EndpointProcessor> endpointClasses = new LinkedHashSet<>();

        ApplicationContext context = getApplicationContext();
        if (context != null) {
            Map<String, Object> processorMap = context.getBeansWithAnnotation(ServerEndpoint.class);
            processorMap.values().forEach(processor -> {
                endpointClasses.add((EndpointProcessor) processor);
            });
        }

        for (EndpointProcessor endpointClass : endpointClasses) {
    ...
    private void registerEndpoint(EndpointProcessor endpointClass) {
        ServerEndpoint annotation = AnnotatedElementUtils.findMergedAnnotation(endpointClass.getClass(), ServerEndpoint.class);
        if (annotation == null) {
        ...
        if (websocketServer == null) {
            PojoEndpointServer pojoEndpointServer = new PojoEndpointServer(endpointClass, serverEndpointConfig, path);
            ...
            websocketServer.getPojoEndpointServer().addPathPojoMethodMapping(path, endpointClass);
        }
    }
